...
- [実行時証拠の観測環境の検証](runtime-environment-validation.md) — 採用した実行時証拠を製品に組み込む前に、観測に必要な権限、常駐時の負荷、実運用環境での開始条件と利用経路を確かめる検証(計画)
- [実行時証拠の網羅性の検証](runtime-coverage-validation.md) — ワークロードが実際に使ったパッケージ全体のうち各方式がどれだけ確認できるかを、独立した正解データで評価する計画
