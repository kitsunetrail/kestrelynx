...
#   Each plan line: <case> <replicate> <sync> <pages>  (case 23, 24, 23+24, 23+host route to attribution-control-run.sh
#   instead, whose third field is pages, not sync). See tools/plans/example.txt.
...
while read -r c rep sync pages; do
  [ -z "$c" ] && continue; case "$c" in \#*) continue;; esac
  echo "$(date -u +%FT%TZ) === $c $rep $sync $pages" | tee -a "$LOGF"
  case "$c" in
    ...
    *) bash "$SNAP/case-run.sh" "$c" "$rep" "$sync" "$pages" > /dev/null 2>&1; R=$(ls -td experiments/runtime-discovery/out/$c-root-*-r$rep-$sync-* 2>/dev/null | head -1);;
  esac
